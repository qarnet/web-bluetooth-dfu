...
export class DfuProvider extends DfuEventTarget {
  ...
  /** @param {object} _session — result from bluetooth/connect.js */
  async attach(_session) {
  ...
  /** @param {File|Uint8Array} _file — validate and parse */
  async loadFirmware(_file) {
    throw new Error('loadFirmware not implemented');
  }

  /** Run the full update. Emits 'log', 'progress', 'phase', 'needs-reconnect'. */
  async runUpdate() {
    throw new Error('runUpdate not implemented');
  }
